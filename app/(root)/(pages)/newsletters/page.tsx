...
const Newsletters = async ( { searchParams }: { searchParams: Promise<{ page?: string }> }) => {
 
  const params = await searchParams;
  const currentPage = Number(params?.page ?? 1);
  const start = (currentPage - 1) * PAGE_SIZE;
  const end = start + PAGE_SIZE;
  const {forms, total} = await client.fetch(newslettersQuery, {start, end});

  const totalPages = Math.ceil(total / PAGE_SIZE);

  return (
    <section className="w-full">
      <div className="relative h-[50vh] md:h-[443px] flex flex-col items-center justify-center p-8 gap-12.5 sm:p-25 bg-[url('/images/newsletter-bg.jpg')] bg-cover bg-center overflow-hidden">
        ...
        <div className="absolute z-10 flex flex-col items-center gap-7.5 text-center p-8">
          <h1 className="text-3xl md:text-5xl font-bold text-white">Newsletters</h1>
          <p className="text-base md:text-lg font-medium text-white text-center">Get the latest news and essential updates from our newsletters.</p>
        </div>
      </div>
