...
import { SendWhistleblowingFormMail } from "@/lib/actions/sendMail"
...
const WhistleblowingForm = () => {
    ...
    async function onSubmit(values: z.infer<typeof contactSchema>) {
        console.log(values)
        const response = await SendWhistleblowingFormMail(values);
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="lg:w-155 bg-white flex flex-col p-7.5 gap-7.5 rounded-[20px] shadow">
                <div className="flex flex-col gap-2.5">
                    ...
                    <p className="text-base font-normal text-grey-100">Fill out the Whistleblower form below</p>
                </div>
                ...
                />
                <Button type="submit" disabled={isSubmitting} className="h-15 cursor-pointer bg-blue-100 hover:bg-yellow-100 text-base font-semibold border hover:border-none border-blue-100 text-white rounded-[8px]">{isSubmitting ? (<><Loader2 className="w-5 h-5 animate-spin" />Submitting...</>) : ("Submit")}</Button>
            </form>
        </Form>
    )
}
